	...
	if [[ $res != "" ]]; then
		...
		do
			if [[ $lecteur == "sd"* ]] && [[ $lecteur != *":" ]] && [[ $lecteur != *"Extended" ]]; then
				if [[ $lecteur == "sda"* ]]; then
					existSda=1
					sizeSda=`df -h | awk '/'$lecteur'/ {print $2}'`
					nbDD=$nbDD+1
				elif [[ $lecteur == "sdb"* ]]; then
					existSdb=1
					sizeSdb=`df -h | awk '/'$lecteur'/ {print $2}'`
					nbDD=$nbDD+1
				elif [[ $lecteur == "sdc"* ]]; then
					existSdc=1
					sizeSdc=`df -h | awk '/'$lecteur'/ {print $2}'`
					nbDD=$nbDD+1
				elif [[ $lecteur == "sdd"* ]]; then
					existSdd=1
					sizeSdd=`df -h | awk '/'$lecteur'/ {print $2}'`
					nbDD=$nbDD+1
				fi
			fi
		done
		...
		if [[ $existSda == 1 ]]; then
			text=$text"sda1 $sizeSda OFF "
		fi
		if [[ $existSdb == 1 ]]; then
			text=$text"sdb1 $sizeSdb OFF "
		fi
		if [[ $existSdc == 1 ]]; then
			text=$text"sdc1 $sizeSdc OFF "
		fi
		if [[ $existSdd == 1 ]]; then
			text=$text"sdd1 $sizeSdd OFF "
		fi
